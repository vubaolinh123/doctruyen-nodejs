...
exports.create = async (req, res) => {
  try {
    const item = new Bookmark(req.body);
    await item.save();
...
exports.update = async (req, res) => {
  try {
    const item = await Bookmark.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(400).json({ error: err.message });
...
exports.remove = async (req, res) => {
  try {
    const item = await Bookmark.findByIdAndDelete(req.params.id);
    if (!item) return res.status(404).json({ error: 'Not found' });
